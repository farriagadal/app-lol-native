...
/**
 * Construye data/<region>/lol.db (SQLite normalizada) a partir del raw store
 * data/<region>/matches.jsonl, usando sql.js (sin compilador nativo).
 * Es la versión in-process de scripts/build-db.mjs, para llamarla tras una
 * recolección lanzada desde la app.
 *
 * `championName` resuelve el id de Data Dragon a partir del championId numérico
 * (para los baneos); en la app se inyecta el del DataDragon ya cargado.
 */
...
export async function buildDb(opts: {
  region: string;
  championName: (numericId: number) => string | null;
  dataDir?: string;
}): Promise<{ matches: number; outPath: string }> {
  const dataDir = opts.dataDir ?? path.resolve(process.cwd(), 'data');
  const jsonl = path.join(dataDir, opts.region, 'matches.jsonl');
  ...
  for await (const line of rl) {
    ...
    for (const p of info.participants as unknown as Array<Record<string, unknown>>) {
      const styles = ((p.perks as { styles?: Array<{ style: number; selections?: Array<{ perk: number }> }> })?.styles) ?? [];
    ...
    for (const team of info.teams) {
      ...
      for (const b of team.bans ?? []) {
        ...
        insBan.run([mid, team.teamId, b.championId, opts.championName(b.championId), b.pickTurn]);
      }
    }
    matches++;
  }
  ...
  const outPath = path.join(dataDir, opts.region, 'lol.db');
